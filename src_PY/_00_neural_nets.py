...
class ConvNet(nn.Module):
    def __init__(self, out_dim, channel_sizes, activation=nn.ReLU):
        ...
        output_dim = get_output_dim(get_output_dim(28, kernel_size=5, stride=1), kernel_size=2, stride=2)
        output_dim = get_output_dim(get_output_dim(output_dim, kernel_size=5, stride=1), kernel_size=2, stride=2)
        
        layer_list = [nn.Conv2d(in_channels=1, out_channels=channel_sizes[0], kernel_size=(5, 5), bias=True),
         nn.MaxPool2d(kernel_size=(2, 2), stride=(2,2)),
         activation(),
         nn.Conv2d(in_channels=channel_sizes[0], out_channels=channel_sizes[1], kernel_size=(5, 5), bias=True),
         nn.MaxPool2d(kernel_size=(2, 2), stride=(2,2)),
         activation(),
         nn.Flatten(),
         nn.Linear(output_dim*output_dim*channel_sizes[-1], out_dim, bias=True)
        ]
