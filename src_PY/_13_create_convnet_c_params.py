...
if __name__ == '__main__':
    ...
    saved_stats = torch.load(args.save_dir/args.filename,weights_only=False)
    state_dict = saved_stats['state_dict']
    channel_sizes = saved_stats['channel_sizes']

    H1_conv = get_output_dim(28, kernel_size=5, stride=1)
    W1_conv = H1_conv
    H1_pool = get_output_dim(H1_conv, kernel_size=2, stride=2)
    W1_pool = H1_pool

    H2_conv = get_output_dim(H1_pool, kernel_size=5, stride=1)
    W2_conv = H2_conv
    ...
    with open('../src/convnet_params.h', 'w') as f:
        ...
        for layer_idx in range(1, 4):
            ...
            # ★ [추가] Bias 선언 (extern)
            # ==========================================
            if f'layer_{layer_idx}_bias' in state_dict:
                name_b = f'layer_{layer_idx}_bias'
                param_b = state_dict[name_b]
                # Bias는 int8이 아니라 int (혹은 int32)로 선언해야 안전합니다.
                f.write(f"extern const int {name_b}[{len(param_b.flatten())}];\n")
            # ==========================================
    ...
    with open('../src/convnet_params.c', 'w') as f:
        ...
        for layer_idx in range(1, 4):
            # 1. Weight 작성 (기존 코드)
            name = f'layer_{layer_idx}_weight'
            # (★ 중요: 여기서 Weight Transpose 로직도 넣어주시면 좋습니다!)
            param = state_dict[name]
            # Conv Layer(1,2번)인 경우만 순서 변경 (HWC -> CHW)
            if layer_idx < 3: 
                 # 파이썬 저장 시 (Out, In, H, W) 형태가 아니라면 
                 # 여기서 .transpose(...) 등을 수행해야 함. 
                 # 일단 기존 로직 유지하고 Bias만 설명하겠습니다.
                 pass
            ...
            if f'layer_{layer_idx}_bias' in state_dict:
                name_b = f'layer_{layer_idx}_bias'
                param_b = state_dict[name_b].flatten()
                
                f.write(f"const int {name_b}[{len(param_b)}] = {{")
                for idx in range(len(param_b)):
                    f.write(f"{param_b[idx]}")
                    if idx < len(param_b) - 1:
                        f.write(", ")
                f.write("};\n\n")
            # ==========================================
