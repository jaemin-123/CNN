...
class ConvNet(nn.Module):
    def __init__(self, out_dim, channel_sizes, activation=nn.ReLU):
        ...
        layer_list = [
            # bias=True를 모두 bias=False로 변경합니다.
            nn.Conv2d(in_channels=1, out_channels=channel_sizes[0], kernel_size=(5, 5), bias=False), 
            nn.MaxPool2d(kernel_size=(2, 2), stride=(2,2)),
            activation(),
            nn.Conv2d(in_channels=channel_sizes[0], out_channels=channel_sizes[1], kernel_size=(5, 5), bias=False),
            nn.MaxPool2d(kernel_size=(2, 2), stride=(2,2)),
            activation(),
            nn.Flatten(),
            # 마지막 Linear 레이어도 bias=False로 설정합니다.
